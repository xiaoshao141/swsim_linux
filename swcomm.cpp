...
swcomm::swcomm()//进行TCP通讯
{
    m_pSkt = new QTcpSocket(this);

    m_LastReportTime = QDateTime::currentDateTime().toTime_t();//上次上报的时间
...
{
    QByteArray ba = m_pSkt->readAll();//ba为读取得到数据
...
void swcomm::sndData()
{
    ...
    quint16 l2 = str.mid(6, 4).toInt();//l2为地址

    m_DZAddr[3] = l2&0xFF;//将低八位写入地址中
    m_DZAddr[4] = l2>>8;//将高八位写入地址中
...
void swcomm::setData(QString dzId, float flow, float level, float flowRate, float gatage, float voltage)
{
      m_DZId = dzId;
      m_Flow = flow;
      m_Level = level;
