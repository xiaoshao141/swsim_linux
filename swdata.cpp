...
swdata::swdata()
{
    m_DataBase = QSqlDatabase::addDatabase("QSQLITE", "swDataDb");//使用驱动程序类型和连接名称connectionName将数据库添加到数据库连接列表中。
...
unsigned int swdata::getDZNum()//获取数据的个数
{
...
QString swdata::getDZName(unsigned int index_id)
{
    ...
    sqlStr.sprintf("select dzname from swdata where id==%d", index_id);//dzname为首行的名字
...
float swdata::getLevel(unsigned int index_id)//index_id为某行
{
    float level;
    QSqlQuery query(m_DataBase);
    QString sqlStr;
